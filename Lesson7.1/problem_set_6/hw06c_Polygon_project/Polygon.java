...
public class Polygon
{
    ...
    int len; // the length of the points list
    public Polygon() { points = new ArrayList<Point>(); len = 0; }

    public void add(Point p) { points.add(p); len++; }

    public double perimeter() {
        double sum = 0.0;
        for(int i = 0; i < len; i++)
            sum += points.get(i).distance(points.get((i+1)%len));
        return sum;
    }
    
    public void draw() {
        for(int i = 0; i < len; i++)
            new Line( points.get(i).x,
                      points.get(i).y,
                      points.get((i+1)%len).x,
                      points.get((i+1)%len).y ).draw();
    }
}
